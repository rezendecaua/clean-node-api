...
describe('Login Router', () => {
  ...
  test('Should return 401 when invalid credentials are provided', async () => {
    ...
    expect(httpReponse.body).toEqual(new UnauthorizedError())
  })
  ...
  test('Should return 200 whhen valid credentials are provided', async () => {
    const { sut, authUseCaseSpy } = makeSut()
    const httpRequest = {
      body: {
  ...
  test('Should return 500 if no AuthUseCase has no isValid method', async () => {
    const authUseCaseSpy = makeAuthUseCase()
    const sut = new LoginRouter(authUseCaseSpy, {})
    const httpRequest = {
      body: {
  ...
  test('Should return 500 if no EmailValditor throws', async () => {
    const authUseCaseSpy = makeAuthUseCase()
